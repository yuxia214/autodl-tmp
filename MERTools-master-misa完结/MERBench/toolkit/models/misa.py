...
class MISA(nn.Module):
    def __init__(self, args):
        ...
        # 可学习的缺失模态token
        self.missing_token_t = nn.Parameter(torch.randn(1, hidden_dim) * 0.02)
        self.missing_token_v = nn.Parameter(torch.randn(1, hidden_dim) * 0.02)
        self.missing_token_a = nn.Parameter(torch.randn(1, hidden_dim) * 0.02)
        # =============================================
    ...
    def modal_dropout(self, utterance_t, utterance_v, utterance_a):
        ...
        if self.training and self.modal_dropout_prob > 0:
            ...
            # 确保每个样本至少保留一个模态
            all_zero_mask = ((mask_t + mask_v + mask_a) == 0).squeeze()
            if all_zero_mask.any():
                # 随机选择一个模态保留
                random_modality = torch.randint(0, 3, (all_zero_mask.sum().item(),), device=device)
