...
MERBENCH_ROOT = os.path.dirname(SCRIPT_DIR)  # 假设脚本在 MERBench/attention_robust_v4/
...
def check_paths():
    ...
    if not os.path.exists(TOOLKIT_MODELS):
        ...
        print(f"请确保脚本放置在 MERBench/attention_robust_v4/ 目录下")
        return False
...
def copy_files():
    ...
    # 复制 attention_robust_v4.py
    src_model = os.path.join(SCRIPT_DIR, 'attention_robust_v4.py')
    dst_model = os.path.join(TOOLKIT_MODELS, 'attention_robust_v4.py')
    if os.path.exists(src_model):
        shutil.copy2(src_model, dst_model)
        print(f"✓ 复制 attention_robust_v4.py -> {dst_model}")
    else:
...
def update_model_init():
    ...
    # 检查是否已经添加
    if 'attention_robust_v4' in content:
        print("✓ attention_robust_v4 已在 __init__.py 中注册")
        return True
    
    # 添加导入
    import_line = "from .attention_robust_v4 import AttentionRobustV4"
    map_line = "    'attention_robust_v4': AttentionRobustV4,"
...
def update_data_init():
    ...
    if 'attention_robust_v4' in content:
        print("✓ attention_robust_v4 已在 data/__init__.py 中注册")
        return True
    
    # 添加数据集映射
    map_line = "    'attention_robust_v4': Data_Feat,"
...
def verify_installation():
    ...
    try:
        ...
        if 'attention_robust_v4' in MODEL_MAP:
            print("✓ AttentionRobustV4 已注册到 MODEL_MAP")
        else:
            print("✗ AttentionRobustV4 未在 MODEL_MAP 中")
            return False
    except ImportError as e:
...
def main():
    ...
    if verify_installation():
        print("\n✓ 所有组件安装成功!")
        print("\n使用方法:")
        print("  python main-robust.py --model='attention_robust_v4' --use_vae ...")
    else:
        print("\n警告: 验证失败，请检查错误信息")
