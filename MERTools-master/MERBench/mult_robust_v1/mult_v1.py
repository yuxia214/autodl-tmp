...
class MULT(nn.Module):
    def __init__(self, args):
        ...
        # Gated Fusion
        self.gate_l = nn.Linear(2 * self.hidden_dim, 2 * self.hidden_dim)
        self.gate_a = nn.Linear(2 * self.hidden_dim, 2 * self.hidden_dim)
        self.gate_v = nn.Linear(2 * self.hidden_dim, 2 * self.hidden_dim)
    ...
    def forward(self, batch):
        ...
        # Gated Fusion
        g_l = torch.sigmoid(self.gate_l(last_h_l))
        g_a = torch.sigmoid(self.gate_a(last_h_a))
        g_v = torch.sigmoid(self.gate_v(last_h_v))
        
        # Add residual connection to gated fusion to prevent vanishing gradients
        last_h_l = last_h_l * g_l + last_h_l
        last_h_a = last_h_a * g_a + last_h_a
        last_h_v = last_h_v * g_v + last_h_v
