#!/bin/bash
# AttentionRobust V8 training script

set -euo pipefail

MERBENCH_ROOT="/root/autodl-tmp/MERTools-master/MERBench"
OUTPUT_DIR="${MERBENCH_ROOT}/attention_robust_v8/outputs"
...
# V7 inherited robustness settings
...
# V8 new settings
USE_GATED_UNCERTAINTY=true
GATE_ALPHA=0.55
FUSION_RESIDUAL_SCALE=0.45
RELIABILITY_TEMPERATURE=0.9
MODALITY_AGREEMENT_WEIGHT=0.012
WEIGHT_CONSISTENCY_WEIGHT=0.025
...
LOG_FILE="${OUTPUT_DIR}/logs/train_v8_${TIMESTAMP}.log"

cd "${MERBENCH_ROOT}"

echo "=============================================="
echo "AttentionRobustV8 Training Start"
echo "Time: ${TIMESTAMP}"
echo "Log: ${LOG_FILE}"
echo "=============================================="

python -u main-robust.py \
    --model='attention_robust_v8' \
    --dataset="${DATASET}" \
    --feat_type="${FEAT_TYPE}" \
...
echo "=============================================="
echo "AttentionRobustV8 Training End"
echo "=============================================="
