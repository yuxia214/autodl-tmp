...
# Add the parent directory to sys.path so that 'toolkit' can be found
...
def train_or_eval_model(args, model, reg_loss, cls_loss, dataloader, epoch, optimizer=None, train=False, dataloader_class=None):
    ...
    for iter, data in enumerate(dataloader):
        ...
        if train:
            loss.backward()
            if model.grad_clip != -1:
                torch.nn.utils.clip_grad_value_([param for param in model.parameters() if param.requires_grad], model.grad_clip)
            optimizer.step()
